...
#  Installation section
    ...
    ##############################################################################################################################
    # Secure Web Access Gateway (SWAG) - nginx reverse proxy.  Set this one up first because all the other web services
    # created later use it and its configuration files.
    ...
    ##############################################################################################################################
    # Farside rotating redirector written in elixer by ben busby - https://github.com/benbusby/farside
    ...
    # Neko Firefox browser
        ...
        docker exec -i $(sudo docker ps | grep $containername | awk '{print $NF}') bash -c 'echo "lockPref(\"identity.sync.tokenserver.uri\", \"https://aqj9z.mine.nu/f4c4hm/token/1.0/sync/1.5\");" >> /usr/lib/firefox/mozilla.cfg'
    ...
    # Whoogle
        ...
        $containername:
            ...
            #user: '102' # user debian-tor from tor package
        $ymlftr" >> $ymlname
    ...
    # Pihole - do this last or it may interrupt you installs due to blacklisting
        ...
        $containername:  # See this link for some help getting the host configured properly or else there will be a port 53 conflict
            ...
            networks:
                ...
                #  Set a static ip address for the pihole
                #  https://www.cloudsavvyit.com/14508/how-to-assign-a-static-ip-to-a-docker-container/
                internet:
